...
The main function to call is `download_10k(driver, ticker, form_type, year_selection, out_dir, verbose)`
...
def download_sec_forms(driver, ticker, form_type, year_selection, out_dir, verbose=False):
    ...
    try:
        ...
        if verbose: print(f'No page exists for {ticker}')
        return
    ...
    if isinstance(year_selection, str):
        ...
        selected_indices = [i for i in range(1, num_rows) if int(results_table_entry(i, -2).text.split('-')[0]) >= start_year]
        ...
        selected_indices = [i for i in range(1, num_rows) if (int(results_table_entry(i, -2).text.split('-')[0]) >= start_year and
                                                              int(results_table_entry(i, -2).text.split('-')[0]) <= end_year)]
    else:
        selected_indices = list(range(1, num_rows))
        
    if verbose: print(f'{ticker}: Search returned {len(selected_indices)} files.')
    ...
    for index in selected_indices:
        ...
        if Path(out_dir + '/' + out_file).is_file():
            if verbose: print(f'\tFile {out_dir + "/" + out_file} already exists.')
            break
        ...
        try:
            ...
            if verbose: print('Click interrupted, waiting to try again')
            time.sleep(5)
        ...
        try:
            ...
            if menu_dropdown:
                ...
                if verbose: print(f'\tSaving {out_dir + "/" + out_file} ...', end='')
            ...
            # Others take you straight to the html form.
                ...
                pdfkit.from_url(driver.current_url, out_dir + '/' + out_file)
                if verbose: print(f'\tSaved {out_dir + "/" + out_file}')
                
        except:
            if verbose: print(f'\tUnable to save {out_dir + "/" + out_file}')
